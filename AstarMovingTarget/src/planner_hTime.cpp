...
std::priority_queue<std::shared_ptr<Node>, std::vector<std::shared_ptr<Node>>, Compare> open_list;
        ...
        )
{
    ...
    {
        std::shared_ptr<Node> s = open_list.top();
        open_list.pop();
        if(heuristics.find(s->mapIndex) != heuristics.end()) continue; // skip repetitions in open list
        heuristics[s->mapIndex] = std::pair<int, int>(s->g, s->time); // closed_list list. stores optimal g-vals
        ...
        )
{
    while(!open_list.empty())
    {
        ...
        if(closed_list.find(newIndexForMap) != closed_list.end()) continue; // skip repetitions in open list
        closed_list.insert(newIndexForMap);
        ...
        for(int dir = 0; dir < (NUMOFDIRS + 1); ++dir)
        {
            ...
            if(newx >= 1 and newx <= x_size and newy >= 1 and newy <= y_size and closed_list.find(newIndexForMap) == closed_list.end())
            {
                ...
                if((cost >= 0) and (cost < collision_thresh)) // cell is free
                {
                    if(Nodes.find(newIndexForMap) == Nodes.end()) // create a new Node, if it does not exist
                    {
                        ...
                        // if newIndex is in goals, then  h = cost*(goalTime - totalTime)
                    ...
                    if(Nodes[newIndexForMap]->g > s->g + cost) // compare g values and cost, update parent if needed
                    {
                        ...
                        open_list.push(Nodes[newIndexForMap]);
                    }
                }
            }
        }
        ...
        )
{
    ...
    prevX = robotposeX; // store current robot position
    prevY = robotposeY;

    if(!initialized) // init s_start, g(start) = 0, add to the open set, and Node map
    {
