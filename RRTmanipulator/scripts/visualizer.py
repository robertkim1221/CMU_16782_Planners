...
def viz():
    ...
    output_dir = "../output/"
    # check if ouput is in args.fileWithSolution already (does not necessarily starts with output/)
    if output_dir in args.fileWithSolution:
        output_dir = ""
    with open(output_dir + args.fileWithSolution) as f:
    ...
    ani.save(output_dir + args.gifFilepath, dpi=300, writer=PillowWriter(fps=args.fps))
